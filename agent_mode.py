...
def _handle_open_app(args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        # We intentionally use Popen directly (not _run_with_timeout) because
        # launching an app is fire-and-forget, not something we want to wait
        # for with a timeout.  We only want to confirm the process *started*.
        subprocess.Popen(
            cmd,
